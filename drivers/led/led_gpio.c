...
static struct led_gpio_data *led_parameters_get(struct device *dev, uint32_t led)
{
...
static int gpio_led_set_no_delay(struct led_gpio_data *data, bool state)
{
	...
	if (data->cfg->gpio.polarity & GPIO_OUTPUT_ACTIVE) {
		pin_value = (state ? 1 : 0);
	} else {
		pin_value = (state ? 0 : 1);
	}
...
static int gpio_led_init(struct device *dev)
{
	...
	for (size_t i = 0; i < cfg->led_cnt; i++) {
		...
		if (cfg->led[i].gpio.polarity & GPIO_OUTPUT_ACTIVE) {
			pin_value = 0;
		} else {
			pin_value = 1;
		}

		err = gpio_pin_set(data[i].gpio_dev, cfg->led[i].gpio.pin,
				   pin_value);
		if (err) {
			LOG_DBG("Failed to turn off %s", cfg->led[i].label);
		}
...
static const struct led_dts_cfg led_dts[] = {DT_INST_FOREACH_CHILD(0, LED_DATA)};
